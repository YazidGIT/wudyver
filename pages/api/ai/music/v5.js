...
const createSong = async (lyrics, style, title) => {
  try {
    const response = await axios.post("https://aisonggenerator.ai/api/create-suno-damo", {
      customMode: true,
      instrumental: false,
      prompt: "",
...
const getMyMusic = async () => {
  try {
    const response = await axios.get("https://aisonggenerator.ai/api/my-music", {
      headers: {
        accept: "*/*",
        "accept-language": "id-ID,id;q=0.9",
        "cache-control": "no-cache",
