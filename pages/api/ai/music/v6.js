...
class AiMusicGen {
  constructor() {
    ...
    // Default nilai untuk generate lagu
    ...
    // Default nilai untuk daftar lagu
  ...
  async generate(data = {}) {
    try {
      const requestData = { ...this.defaultGenerateData, ...data };
      const response = await axios.post(this.apiMusicGen, requestData, { headers: this.headers });
  ...
  async fetchMusicList(data = {}) {
    try {
      const { userId, status, offset, limit } = { ...this.defaultListData, ...data };
      const url = `${this.apiSupabase}?select=*&user_id=eq.${userId}&status=eq.${status}&order=created_at.desc&offset=${offset}&limit=${limit}`;
      const response = await axios.get(url, { headers: this.supabaseHeaders });
      return response.data;
    } catch (error) {
  ...
  switch (action) {
    case "create":
      try {
        ...
        if (!song) return res.status(500).json({ error: "Gagal membuat lagu" });
    ...
    case "list":
      try {
        ...
        if (!musicList) return res.status(500).json({ error: "Gagal mengambil daftar lagu" });
    ...
    default:
      return res.status(400).json({ error: "Action tidak valid. Gunakan ?action=create atau ?action=list" });
  }
}
